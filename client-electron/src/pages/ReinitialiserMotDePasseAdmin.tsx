...
import type { PatronResume } from "../api/types";
 ...
 * flux par email (code envoyé au Patron) reste séparé, voir MotDePasseOublie.tsx,
 * accessible directement depuis Connexion.tsx quand la boutique a la synchro
 * activée.
 *
...
export default function ReinitialiserMotDePasseAdmin({
  ...
  useEffect(() => {
    api.auth.listerPatrons(username, password).then(setPatrons);
  ...
  async function soumettre(evenement: React.FormEvent) {
    ...
    setEnCours(true);
    try {
      const resultat = await api.auth.reinitialiserMotDePasseAdmin(
        username,
        password,
        usernameCible,
        nouveauMotDePasse,
