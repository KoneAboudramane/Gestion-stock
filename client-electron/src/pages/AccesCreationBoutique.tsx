...
/**
 * Verrou devant "Créer une boutique" (voir Connexion.tsx, point d'accès
 * caché derrière la petite icône) : demande les identifiants d'un compte
 * administrateur (is_staff côté Django, voir comptes/views.py::VerifierAccesAdminView)
 * avant de révéler les actions ci-dessous — réservé à l'exploitant de la
 * plateforme, pas aux commerçants qui se connectent normalement ici. Ne
 * touche jamais à la session déjà active sur cet appareil. Pas de
 * PanneauMarque (panneau marketing pensé pour les commerçants) : cet écran
 * n'a de sens que pour l'exploitant, aucune raison de le lui montrer.
 *
 ...
 * ici est le repli sans email/code (identité déjà prouvée par ce verrou) ;
 * le flux par email classique (voir MotDePasseOublie.tsx) reste accessible
...
export default function AccesCreationBoutique({ allerConnexion }: { allerConnexion: () => void }) {
  ...
  useEffect(() => {
    ...
    // pendant que ce poste était hors-ligne (voir auth.ts::verifierRevocationAdmin).
