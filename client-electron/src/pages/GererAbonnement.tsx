...
import type { AbonnementBoutique, ChampsAbonnement } from "../api/types";
 ...
 * agit sur la boutique déjà installée sur ce poste (un poste = une boutique).
 * Même patron vue/édition que Réglages > Informations boutique : on affiche
 * d'abord l'état actuel, "Modifier" bascule sur le formulaire — pensé pour le
 * cas où l'exploitant est chez le commerçant sans internet, la modification
 * s'applique toujours en local immédiatement, et se synchronise avec le
 * serveur dès que possible (tout de suite si le réseau répond, sinon voir
 * Réglages > Synchronisation pour la pousser plus tard). Réutilise les
 * identifiants déjà saisis pour franchir le verrou admin — jamais redemandés
 * ici.
 *
...
export default function GererAbonnement({
  ...
  useEffect(() => {
    api.admin.boutiqueLocale().then((b) => {
      setBoutique(b);
  ...
  async function soumettre(evenement: React.FormEvent) {
    ...
    try {
      ...
      setMessage(
        reponse.resultat.statut === "synchronise"
