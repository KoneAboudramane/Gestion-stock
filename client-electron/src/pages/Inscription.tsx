...
/**
 * S'ouvre en modale depuis AccesCreationBoutique.tsx (seul point d'entrée
 * depuis la refonte du 2026-09-01 — plus d'écran plein écran, plus de
 * PanneauMarque : cet écran n'a de sens que pour l'exploitant caché derrière
 * l'icône ⚙ de Connexion.tsx).
 *
 ...
 * - "Hors ligne" : créée directement en local (voir electron/services/
 *   inscriptionLocale.ts) — utilisable tout de suite, même sans réseau.
 *   "Activer en ligne" reste alors un geste séparé, voir Réglages >
 *   Synchronisation.
 */
export default function Inscription({
  ...
  async function soumettre(evenement: React.FormEvent) {
    ...
    try {
      if (mode === "enLigne") {
        const resultat = await api.admin.creerBoutiqueEnLigne(usernameAdmin, passwordAdmin, {
          boutiqueNom,
          username,
          password,
          email,
        });
        if (resultat.succes) setStatutFinal(resultat.resultat.statut);
        else setErreur(resultat.message);
      } else {
        const resultat = await api.admin.creerBoutiqueLocale({ boutiqueNom, username, password, email });
        if (resultat.succes) setStatutFinal("horsLigne");
        else setErreur(resultat.message);
      }
    } finally {
      setEnCours(false);
