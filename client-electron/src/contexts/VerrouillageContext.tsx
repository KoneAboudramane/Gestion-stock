...
import type { ParametreResume, Session } from "../api/client";

// Port de client-web/src/contexts/VerrouillageContext.tsx : même rôle,
// paramètre configuration.Parametre lu via IPC (electron/services/reglages.ts)
// au lieu d'IndexedDB.
...
export function VerrouillageProvider({ session, children }: { session: Session; children: ReactNode }) {
  const [delaiMinutes, setDelaiMinutes] = useState(DELAI_VERROUILLAGE_PAR_DEFAUT);

  function rafraichir() {
    api.reglages.listerParametres(session.boutiqueId).then((parametres: ParametreResume[]) => {
